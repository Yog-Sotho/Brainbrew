...
class DistillationConfig(BaseModel):
    ...
    def validate_model_names(cls, v: str | None, info: ValidationInfo) -> str | None:
        ...
        # Ensure we block Unix-style absolute paths, Windows-style backslash absolute paths,
        # or Windows drive-letter configurations (e.g. C:/ or D:\).
        is_absolute = (
            v_stripped.startswith("/")
            or v_stripped.startswith("\\")
            or bool(re.match(r"^[a-zA-Z]:", v_stripped))
        )
        if ".." in v_stripped or is_absolute:
