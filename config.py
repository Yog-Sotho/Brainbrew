    ...
    def validate_checkpoint_dir(cls, v: str | None) -> str | None:
        if v is not None:
            ...
            if (
                ...
                or v_stripped.startswith("\\")
                or (len(v_stripped) > 1 and v_stripped[1] == ":" and v_stripped[0].isalpha())
            ):
                raise ValueError(
                    "Checkpoint directory path cannot contain path traversal sequences or absolute local paths."
                )
            if any(ord(c) < 32 or ord(c) > 126 for c in v_stripped):
