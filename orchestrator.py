...
def run_distillation(cfg: DistillationConfig, source_file: Path, progress_callback: Optional[Callable[[int], None]] = None) -> Path:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        pipeline = Pipeline(steps=[
            ...
            # Rename 'generation' → 'output' using output_mappings (no separate step needed)
            TextGeneration(llm=llm, input_mappings={"instruction": "evolved_instruction"}, output_mappings={"generation": "output"}),
            KeepColumns(columns=["instruction", "output"]),
            # Filter on 'output' (now correctly named)
