...
with st.sidebar:
    ...
    use_vllm: bool = st.checkbox(
        ...
        value=True,
        help="Run high-speed local inference using a vLLM engine. Requires an NVIDIA GPU with at least 24GB VRAM.",
    )
...
quality_label: str = st.selectbox(
    ...
    index=1,
    help="Select the balance between generation speed and dataset complexity/quality. 'Research' uses the most complex evolution steps.",
)
...
dataset_size: int = st.slider(
    ...
    20000,
    2000,
    help="The desired total number of synthetic instruction-output instruction pairs to generate.",
)
train_model: bool = st.checkbox(
    "Auto-train LoRA adapter",
    value=False,
    help="Automatically kick off a LoRA fine-tuning run using Unsloth after the synthetic dataset generation completes.",
)
publish: bool = st.checkbox(
    "Publish to Hugging Face",
    value=False,
    help="Upload the generated dataset directly to the Hugging Face Hub once generation completes successfully.",
)
...
uploaded_files = st.file_uploader(
    ...
    help="Drag and drop or browse to upload your custom reference documents. Supported formats: PDF, TXT.",
)
        ...
        try:
            ...
            with open(final_path, encoding="utf-8") as f:
                st.download_button(
                    ...
                    help="Click to download the final synthetic dataset directly to your local machine.",
                )
